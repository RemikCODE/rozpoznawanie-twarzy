...
public partial class MainPage : ContentPage
{
    ...
    public MainPage(ApiService apiService)
    {
        ...
        // Wszystkie platformy mają dostęp do galerii
        GalleryButton.IsVisible = true;

        // Reszta przycisków zależy od platformy
        SelectFileButton.IsVisible = _isDesktop;
        TakePhotoButton.IsVisible = !_isDesktop;

        InstructionLabel.Text = _isDesktop
            ? "wybierz plik z obrazem lub z galerii aby rozpoznac osobe"
            : "zrob zdjecie lub wybierz z galerii aby rozpoznac osobe";
    }

    // NOWA METODA - wybór z galerii
    private async void OnGalleryClicked(object sender, EventArgs e)
    {
        try
        {
            // Prośba o uprawnienia na Android
#if ANDROID
            var readStatus = await Permissions.RequestAsync<Permissions.StorageRead>();
            if (readStatus != PermissionStatus.Granted)
            {
                await DisplayAlert("Brak dostępu", "Aplikacja potrzebuje dostępu do galerii", "OK");
                return;
            }
#endif
    ...
    private async void OnTakePhotoClicked(object sender, EventArgs e)
    {
        try
        {
            ...
            {
                await DisplayAlert("Brak dostępu", "Aplikacja potrzebuje dostępu do kamery", "OK");
                return;
            }
#endif
